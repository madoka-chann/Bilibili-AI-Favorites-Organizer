...
(function() {
    ...
    if (!_cssLoaded) {
        ...
        fallbackCSS.textContent = `
            ...
            .ai-panel-content{background:var(--ai-bg)!important;padding:0;flex:1 1 0%!important;min-height:0!important;max-height:none!important;overflow-y:auto;overflow-x:hidden;border-bottom-left-radius:26px;border-bottom-right-radius:26px;}
...
其中 conf 是你对该分类的置信度(0-1)，1表示非常确定，0.5表示不太确定。`;
    ...
    function initUI() {
        ...
        // === Liquid Crystal v0.0.3 — 液晶流韵新特性（受 animEnabled 控制）===
        if (settings.animEnabled) {
        ...
        // (Cursor Glow Follower 已移除 — 与 Ambient Glow 光标跟随功能重复)
        ...
        (() => {
            ...
            panel.addEventListener('mousemove', (e) => {
                const now = Date.now();
                if (now - lastScatterTime < 80) return;
                lastScatterTime = now;
        ...
        } // end if (settings.animEnabled) — Liquid Crystal 特性结束
        ...
        floatBtn.onclick = (e) => {
            ...
            panel.style.animation = 'ai-panel-in 0.5s var(--ai-spring-gentle) forwards';
            clampPanelPosition();
        ...
        document.getElementById('ai-close-btn').onclick = () => {
            ...
            panel.style.animation = 'none';
            void panel.offsetHeight;
            panel.style.animation = 'ai-panel-out 0.35s var(--ai-ease-smooth) forwards';
            setTimeout(() => {
                panel.style.display = 'none';
                panel.style.animation = 'none';
                panel.classList.remove('ai-panel-closing');
                ...
                floatBtn.style.animation = 'ai-scale-in 0.35s var(--ai-spring-gentle)';
            }, 350);
        };
