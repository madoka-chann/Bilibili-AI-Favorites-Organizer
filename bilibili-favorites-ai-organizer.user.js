// ==UserScript==
// @name         B站 AI 收藏夹自动细化整理 (V21.0 增强版)
// @namespace    http://tampermonkey.net/
// @version      21.0.0
// @description  支持 Gemini/OpenAI/DeepSeek/通义千问/Claude/OpenRouter 等多家 AI 智能分类收藏夹视频 | 自定义模板/增量整理/定时自动整理/AI费用估算/分类导出CSV&JSON&HTML报告/收藏夹健康报告/置信度可视化&低置信度筛选/失效视频批量归档/抓取缓存/动态System Prompt/Token用量追踪/标题栏进度/智能碎片合并/跨收藏夹去重/分类合并/AI自动重试/遗漏检测/全局防风控冷却/可拖拽按钮/XSS安全/撤销历史栈/备份/自适应限速
...
(function() {
    ...
    function exportSettings() {
        ...
        const exported = { ...s, apiKey: '', _exportTime: new Date().toISOString(), _version: '21.0' };
    ...
    function initUI() {
        ...
        document.getElementById('ai-tool-help').onclick = () => {
            const faqs = [
                ...
                { q: '增量整理是什么？', a: 'V20.0 新增。在行为设置中勾选"增量整理"，下次运行时只处理上次整理之后新收藏的视频，跳过已整理过的内容，大幅提高效率。' },
                { q: '自定义模板怎么用？', a: 'V20.0 新增。在 Prompt 输入框旁点击 ⭐ 保存当前规则为命名模板，⚙️ 管理已保存的模板。模板会持久化存储，随时调用。' },
                { q: '收藏夹健康检查', a: 'V20.0 新增。点击 ❤️ 按钮一键扫描所有收藏夹，生成健康评分（0-100）、失效率、重复率、分布均匀度等报告，并给出优化建议。' },
                { q: '导出分类结果', a: 'V20.0 新增。在预览界面点击 ⬇️ 按钮，可将分类结果导出为 CSV 或 JSON 文件，方便后续分析或分享。' },
                ...
                { q: 'AI 请求失败会自动重试吗？', a: '是的！V16.0 新增了自动重试机制，遇到网络超时、API限流(429)、服务过载(503)等错误会自动进行最多3次重试，采用指数退避策略（2秒、4秒、8秒）。' },
                { q: '重复视频可以一键清理吗？', a: '可以！点击"查找重复"扫描全部收藏夹后，可一键去重。跨收藏夹整理时，预览界面也会自动检测重复并提供清理选项。' },
                { q: '失效视频归档功能', a: 'V21.0 新增。点击"失效归档"按钮，扫描所有收藏夹中的失效视频，可选择移动到专用「失效视频归档」收藏夹或直接删除。移动到专用收藏夹便于日后查看是否恢复。' },
                { q: '低置信度筛选', a: 'V21.0 新增。在预览界面点击"⚠️ 低置信度"按钮，筛选出包含低置信度（< 70%）视频的分类，并高亮标记，方便逐一审查 AI 不确定的分类结果。' },
                { q: '定时自动整理', a: 'V21.0 新增。在行为设置中勾选"定时整理"并设置间隔，脚本会自动使用增量模式定期整理新收藏的视频。需要页面保持打开。' },
                { q: '整理报告导出', a: 'V21.0 新增。整理完成后或在预览界面点击 📄 按钮，可导出 HTML 格式的整理报告，包含分类概览、置信度统计、UP主排行等详细信息。' }
            ];
