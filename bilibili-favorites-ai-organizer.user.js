...
// @version      1.6.5
// @description  支持所有AI智能分类B站收藏夹视频 | 自定义模板/增量整理/AI费用估算/分类导出CSV&JSON&HTML报告/收藏夹健康报告/置信度可视化&低置信度筛选/失效视频批量归档/抓取缓存/动态System Prompt/Token用量追踪/标题栏进度/智能碎片合并/跨收藏夹去重/分类合并/AI自动重试/遗漏检测/全局防风控冷却/可拖拽按钮/XSS安全/撤销历史栈/备份/自适应限速/Toast通知/Confetti庆祝动画/键盘快捷键/整理历史时间线/极光渐变UI/毛玻璃面板
...
// @grant        GM_addStyle
// @resource     customCSS https://raw.githubusercontent.com/madoka-chann/Bilibili-AI-Favorites-Organizer/refs/heads/main/bilibili-favorites-ai-organizer.css
...
(function() {
    ...
    if (!_cssLoaded) {
        ...
        fallbackCSS.textContent = `
            ...
            .ai-header-btn:hover{background:rgba(255,255,255,0.3);transform:scale(1.12) translateY(-1px);}
            ...
            .ai-group-header{display:flex;align-items:center;gap:8px;padding:9px 10px;margin:3px -10px;cursor:pointer;font-size:12px;font-weight:600;color:var(--ai-text-secondary);user-select:none;border-radius:var(--ai-radius-md);transition:all 0.3s var(--ai-ease-ios);}
            ...
            .ai-btn{transition:all 0.3s var(--ai-ease-ios);cursor:pointer;border:1.5px solid var(--ai-border);border-radius:var(--ai-radius-sm);background:var(--ai-bg);display:inline-flex;align-items:center;justify-content:center;gap:5px;font-weight:500;}
            ...
            .ai-btn-primary{background:linear-gradient(135deg,#7364FF,#9B59F6,#7364FF)!important;background-size:500% 500%;color:#fff!important;border:none;font-weight:700;position:relative;overflow:hidden;}
            .ai-btn-primary:hover{box-shadow:0 12px 32px rgba(115,100,255,0.30),0 5px 16px rgba(255,107,157,0.16);}
            .ai-btn-tool{background:var(--ai-bg-tertiary);color:var(--ai-text-secondary);font-size:11px;font-weight:500;}
            .ai-btn-tool:hover{transform:translateY(-3px) scale(1.045);box-shadow:0 10px 28px rgba(115,100,255,0.12);}
            ...
            .ai-log-entry{display:flex;align-items:flex-start;gap:6px;padding:3px 8px;border-radius:6px;background:var(--ai-bg-secondary);border-left:3px solid transparent;animation:ai-log-flash 0.6s ease-out;line-height:1.5;}
            ...
            .ai-log-success{color:#059669;border-left-color:var(--ai-success);}
            .ai-log-info{color:var(--ai-info);border-left-color:var(--ai-info);}
            .ai-modal-backdrop{position:fixed;inset:0;background:var(--ai-modal-backdrop)!important;z-index:var(--ai-z-modal);display:flex;align-items:center;justify-content:center;font-family:var(--ai-font);backdrop-filter:blur(16px) saturate(1.4);}
            .ai-modal{background:var(--ai-bg)!important;color:var(--ai-text)!important;border-radius:28px;box-shadow:0 44px 110px rgba(0,0,0,0.2),0 18px 44px rgba(0,0,0,0.11);width:min(600px,90vw);max-height:70vh;display:flex;flex-direction:column;overflow:hidden;}
            .ai-modal-header{padding:18px 22px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.10);background:linear-gradient(135deg,#7364FF,#9B59F6,#7364FF);background-size:800% 800%;color:#fff;position:relative;overflow:hidden;}
            ...
            .ai-cat-block{animation:ai-slide-in 0.55s var(--ai-ease-ios) both;animation-delay:calc(var(--i,0)*0.045s);}
            .ai-cat-row{display:flex;align-items:center;gap:10px;padding:14px 20px;border-bottom:1px solid var(--ai-border-lighter);border-left:3px solid var(--ai-primary);cursor:pointer;position:relative;overflow:hidden;transition:all 0.4s var(--ai-ease-ios);}
            .ai-cat-row:hover{background:var(--ai-bg-hover);transform:translateX(3px);}
            .ai-cat-row.unchecked{opacity:0.3;transform:translateX(6px) scale(0.98);border-left-color:var(--ai-border);}
            ...
            .ai-cat-badge{font-size:10px;color:#fff;background:linear-gradient(135deg,#F43F5E,#FB7185);padding:2px 11px;border-radius:10px;font-weight:600;}
            .ai-cat-existing{font-size:10px;color:#fff;background:linear-gradient(135deg,#059669,#10B981);padding:2px 11px;border-radius:10px;font-weight:600;}
            ...
            .ai-vid-item{padding:7px 20px 7px 48px;font-size:12px;color:var(--ai-text-secondary);line-height:1.5;position:relative;transition:all 0.2s ease;}
            ...
            .ai-vid-cover{width:64px;height:36px;border-radius:var(--ai-radius-xs);object-fit:cover;flex-shrink:0;background:var(--ai-border-lighter);transition:transform 0.35s var(--ai-spring),box-shadow 0.3s ease;}
            ...
            .ai-vid-title a{color:var(--ai-text);text-decoration:none;transition:color 0.2s;}
            ...
            .ai-filter-btn.active{background:linear-gradient(135deg,#7C5CFC,#D946EF,#FF6B8A);color:#fff;border-color:transparent;box-shadow:0 5px 18px rgba(124,92,252,0.24);}
            ...
            .ai-ripple-dot{position:absolute;width:20px;height:20px;border-radius:50%;background:rgba(124,92,252,0.22);pointer-events:none;z-index:2;}
            .ai-particle-dot{position:fixed;border-radius:50%;pointer-events:none;z-index:var(--ai-z-particle);}
            .ai-glow{position:absolute;width:180px;height:180px;background:radial-gradient(circle,var(--ai-glow-color),transparent 70%);pointer-events:none;transform:translate(-50%,-50%);transition:opacity 0.3s;opacity:0;z-index:0;}
            ...
            @keyframes ai-slide-in{0%{transform:translateX(36px) scale(0.93);opacity:0;filter:blur(2.5px)}30%{transform:translateX(-5px) scale(1.015);opacity:1;filter:blur(0)}52%{transform:translateX(2px) scale(0.997)}100%{transform:translateX(0) scale(1);opacity:1}}
            @keyframes ai-fade-in{from{opacity:0}to{opacity:1}}
            @keyframes ai-scale-in{from{transform:scale(0.93);opacity:0}to{transform:scale(1);opacity:1}}
            ...
            @keyframes ai-breathe{0%,100%{box-shadow:0 4px 14px var(--ai-primary-shadow)}50%{box-shadow:0 4px 28px rgba(124,92,252,0.45)}}
            @keyframes ai-panel-in{0%{transform:translateY(40px) scale(0.9);opacity:0;filter:blur(6px)}20%{transform:translateY(-5px) scale(1.018);opacity:1;filter:blur(0.3px)}36%{transform:translateY(2.5px) scale(0.996);filter:blur(0)}50%{transform:translateY(-1px) scale(1.004)}100%{transform:translateY(0) scale(1);opacity:1;filter:blur(0)}}
            @keyframes ai-panel-out{0%{transform:translateY(0) scale(1);opacity:1;filter:blur(0)}100%{transform:translateY(24px) scale(0.94);opacity:0;filter:blur(3px)}}
            ...
            .ai-countdown-bubble{position:fixed;z-index:2147483647;background:linear-gradient(135deg,#7C5CFC,#D946EF,#FF6B8A);color:#fff;font-size:11px;font-weight:bold;padding:4px 12px;border-radius:14px;white-space:nowrap;pointer-events:none;animation:ai-bubble-pop 0.45s cubic-bezier(0.175,0.885,0.32,1.275) forwards;box-shadow:0 4px 16px rgba(124,92,252,0.34),0 0 0 2px rgba(255,255,255,0.5);line-height:1.4;letter-spacing:0.5px;}
            .ai-countdown-bubble::after{content:'';position:absolute;bottom:-6px;left:50%;transform:translateX(-50%);border:6px solid transparent;border-top-color:#FF6B8A;}
    ...
    function logStatus(msg) {
        ...
        // 根据内容自动着色 + 呼吸脉冲状态联动
        const dot = document.getElementById('ai-breathing-dot');
        if (msg.includes('❌') || msg.includes('失败')) { div.classList.add('ai-log-error'); if (dot) { dot.className = 'ai-breathing-dot error'; } }
        else if (msg.includes('⚠️') || msg.includes('警告')) { div.classList.add('ai-log-warning'); if (dot) { dot.className = 'ai-breathing-dot active'; } }
        else if (msg.includes('✅') || msg.includes('完成') || msg.includes('🎉')) { div.classList.add('ai-log-success'); if (dot) { dot.className = 'ai-breathing-dot idle'; } }
        else if (msg.includes('🧠') || msg.includes('📥') || msg.includes('🚚')) { div.classList.add('ai-log-info'); if (dot) { dot.className = 'ai-breathing-dot active'; } }
        else { if (dot) { dot.className = 'ai-breathing-dot active'; } }
...
h1{text-align:center;color:#7C5CFC;margin-bottom:4px;}
...
.summary-card .num{font-size:28px;font-weight:bold;color:#7C5CFC;}
.summary-card .label{font-size:11px;color:#999;margin-top:4px;}
.section{margin-bottom:30px;}
.section h2{font-size:16px;border-bottom:2px solid #7C5CFC;padding-bottom:6px;margin-bottom:12px;}
    ...
    function renderPreview(allCategories, existingFoldersMap, videoIdMap, sourceMediaId, biliData, crossFolderDups) {
        return new Promise(resolve => {
            ...
            for (const [catName, vids] of catEntries) {
                ...
                <div class="ai-cat-block" data-catname="${safeKey}" data-isnew="${isNew}" style="${animSettings.animEnabled ? '--i:' + catIdx : 'animation:none'}">
                    <div class="ai-cat-row">
                        ...
                        <input type="checkbox" class="ai-mc-check" data-cat="${safeKey}" data-count="${vids.length}" checked style="width:16px;height:16px;cursor:pointer;accent-color:#7C5CFC;">
            ...
            <div class="ai-modal">
                ...
                <div class="ai-modal-toolbar" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                    ...
                    <label style="display:flex;align-items:center;gap:4px;font-size:12px;color:#666;white-space:nowrap;cursor:pointer;">
                        <input type="checkbox" id="ai-mc-selectall" checked style="width:15px;height:15px;accent-color:#7C5CFC;"> 全选
                    </label>
            ...
            function spawnParticles(x, y) {
                if (!animSettings.animEnabled) return;
                const colors = ['#7C5CFC', '#FF6B8A', '#A855F7', '#06B6D4', '#10B981', '#F59E0B', '#D946EF', '#FB7185', '#B4A0FF', '#FBBF24'];
                const count = 36;
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    const borderRadius = Math.random() > 0.6 ? '50%' : (Math.random() > 0.5 ? '2px' : '1px');
                    const delay = (Math.random() * 0.12);
                    dot.style.cssText = `left:${x}px;top:${y}px;width:${size}px;height:${size}px;background:${colors[i % colors.length]};border-radius:${borderRadius};--tx:${Math.cos(angle) * dist}px;--ty:${Math.sin(angle) * dist - 50}px;animation-duration:${0.5 + Math.random() * 0.6}s;animation-delay:${delay}s;opacity:${0.6 + Math.random() * 0.4};box-shadow:0 0 ${size}px ${colors[i % colors.length]}30;`;
...
其中 conf 是你对该分类的置信度(0-1)，1表示非常确定，0.5表示不太确定。`;
    ...
    function resetMainButton() {
        ...
        if (btn) {
            ...
            if (typeof lucide !== 'undefined') lucide.createIcons({nodes:[btn]});
            btn.style.background = 'linear-gradient(135deg, #7C5CFC, #FF6B8A)';
            btn.disabled = false;
    ...
    function launchConfetti(x, y, count = 60) {
        const colors = ['#7C5CFC', '#FF6B8A', '#A855F7', '#06B6D4', '#10B981', '#F59E0B', '#F43F5E', '#D946EF', '#FB7185', '#FBBF24', '#B4A0FF', '#34D399'];
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (shape === 'circle') {
                ...
                piece.style.width = (2.5 + Math.random() * 3) + 'px';
                ...
                piece.style.width = s + 'px';
                piece.style.height = s + 'px';
                piece.style.borderRadius = Math.random() > 0.5 ? '1px' : '0';
            }
            // 物理参数 — 更真实的散射
            piece.style.setProperty('--fall-distance', (280 + Math.random() * 500) + 'px');
            piece.style.setProperty('--rotate-end', (400 + Math.random() * 800) + 'deg');
            piece.style.setProperty('--rotate-x', (Math.random() * 540) + 'deg');
            piece.style.setProperty('--rotate-y', (Math.random() * 360) + 'deg');
            ...
            const dy = Math.sin(angle) * spread * 0.6 - spread * 0.3; // 偏上方散射
            piece.style.transform = `translate(${dx}px, ${dy}px)`;
            piece.style.boxShadow = `0 0 ${2 + Math.random() * 4}px ${color}40`;
    ...
    function initUI() {
            ...
            <div class="ai-panel-content">
                ...
                <div id="ai-settings-area" class="ai-settings">
                    ...
                    <div class="ai-group-header" data-group="ai-group-1">
                        <span class="ai-group-icon" style="background:rgba(124,92,252,0.08);color:#7C5CFC;"><i data-lucide="cpu" style="width:13px;height:13px;"></i></span>
                    ...
                    <div class="ai-group-header" data-group="ai-group-2">
                        <span class="ai-group-icon" style="background:rgba(99,102,241,0.08);color:#6366f1;"><i data-lucide="sliders-horizontal" style="width:13px;height:13px;"></i></span>
                    ...
                    <div class="ai-group-header" data-group="ai-group-3">
                        <span class="ai-group-icon" style="background:rgba(16,185,129,0.08);color:#10b981;"><i data-lucide="toggle-right" style="width:13px;height:13px;"></i></span>
                    ...
                    <div class="ai-group-header" data-group="ai-group-4">
                        <span class="ai-group-icon" style="background:rgba(245,158,11,0.08);color:#f59e0b;"><i data-lucide="sparkles" style="width:13px;height:13px;"></i></span>
                ...
                <div style="padding:12px 15px 15px;">
                    ...
                    <div id="ai-status-log" class="ai-status-log">
                        <span style="display:flex;align-items:center"><span id="ai-breathing-dot" class="ai-breathing-dot idle"></span>等待指令...</span>
                    </div>
        ...
        // === Silk Cascade v0.0.7 — 流光扫掠层 ===
        (() => {
            const shimmer = document.createElement('div');
            shimmer.className = 'ai-shimmer-sweep';
        ...
        (() => {
            ...
            for (let i = 0; i < MOTE_COUNT; i++) {
                ...
                mote.className = 'ai-light-mote';
                const size = 4 + Math.random() * 12;
                const duration = 10 + Math.random() * 14;
                const delay = Math.random() * -duration;
                const r = () => (-30 + Math.random() * 60) + 'px';
                mote.style.cssText = `
                    ...
                    --mote-delay: ${delay}s;
                    --mote-opacity: ${0.06 + Math.random() * 0.10};
                    --mote-dx1: ${r()}; --mote-dy1: ${r()};
                    --mote-dx2: ${r()}; --mote-dy2: ${r()};
                    --mote-dx3: ${r()}; --mote-dy3: ${r()};
                `;
        ...
        (() => {
            ...
            for (let i = 0; i < FIREFLY_COUNT; i++) {
                ...
                el.className = 'ai-firefly';
                const size = 2 + Math.random() * 4;
                el.style.width = size + 'px';
                el.style.height = size + 'px';
                ffCanvas.appendChild(el);
                fireflies.push({
                    el,
                    x: Math.random() * 100,
                    y: Math.random() * 100,
            ...
            const renderFireflies = () => {
                ...
                fireflies.forEach((f, i) => {
                    f.x += f.vx + Math.sin(ffTime * f.speed + f.phase) * 0.03;
                    f.y += f.vy + Math.cos(ffTime * f.speed * 0.7 + f.phase) * 0.025;

                    if (f.x < -2 || f.x > 102) f.vx *= -1;
                    if (f.y < -2 || f.y > 102) f.vy *= -1;
                    f.x = Math.max(-3, Math.min(103, f.x));
                    f.y = Math.max(-3, Math.min(103, f.y));

                    const pulse = 0.3 + Math.sin(ffTime * f.pulseSpeed + f.phase) * 0.3 + Math.sin(ffTime * f.pulseSpeed * 0.4 + f.phase * 2) * 0.15;
                    const color = colors[i % colors.length];
                    const glow = f.size * (2 + Math.sin(ffTime * f.pulseSpeed * 0.5) * 1);
                    ...
                    f.el.style.cssText = `
                        ...
                        box-shadow: 0 0 ${glow.toFixed(1)}px ${color};
                        border-radius: 50%;
                        pointer-events: none;
                        position: absolute;
                    `;
                ...
                ffRaf = requestAnimationFrame(renderFireflies);
            };

            const startFF = () => { if (!ffRaf) renderFireflies(); };
            const stopFF = () => { if (ffRaf) { cancelAnimationFrame(ffRaf); ffRaf = null; } };

            const ffObs = new MutationObserver(() => {
        ...
        (() => {
            ...
            wrap.className = 'ai-aurora-ribbon';
            ...
            const aResize = () => {
                ...
                aCanvas.width = Math.round(r.width * 0.5);
                aCanvas.height = Math.round(r.height * 0.5);
            };
            ...
            const renderAurora = () => {
                ...
                if (!ctx || !aCanvas.width) { aRaf = requestAnimationFrame(renderAurora); return; }
                ...
                ribbons.forEach((r, idx) => {
                    ...
                    grad.addColorStop(0, `rgba(${rgb[0]},${rgb[1]},${rgb[2]}, 0)`);
                    grad.addColorStop(0.2, `rgba(${rgb[0]},${rgb[1]},${rgb[2]}, 0.4)`);
                    grad.addColorStop(0.5, `rgba(${rgb[0]},${rgb[1]},${rgb[2]}, 0.6)`);
                    grad.addColorStop(0.8, `rgba(${rgb[0]},${rgb[1]},${rgb[2]}, 0.4)`);
                    grad.addColorStop(1, `rgba(${rgb[0]},${rgb[1]},${rgb[2]}, 0)`);
                    ...
                    const yDrift = Math.sin(aTime * r.drift * 60 + r.phase) * 0.06;
                    const baseY = (r.yBase + yDrift) * h;

                    ctx.moveTo(0, baseY + Math.sin(r.phase + aTime * r.speed) * r.amplitude * h);

                    for (let x = 2; x <= w; x += 2) {
                        const nx = x / w;
                        const y = baseY
                            + Math.sin(nx * r.frequency * Math.PI * 2 + aTime * r.speed + r.phase) * r.amplitude * h
                            + Math.sin(nx * r.frequency * Math.PI * 3 - aTime * r.speed * 0.5 + r.phase * 1.7) * r.amplitude * h * 0.25
                            + Math.cos(nx * Math.PI + aTime * r.speed * 0.3) * r.amplitude * h * 0.12;
                        ctx.lineTo(x, y);
                    }
                ...
                aRaf = requestAnimationFrame(renderAurora);
            };

            const startAurora = () => { aResize(); if (!aRaf) renderAurora(); };
            const stopAurora = () => { if (aRaf) { cancelAnimationFrame(aRaf); aRaf = null; } };

            const aObs = new MutationObserver(() => {
                if (panel.style.display !== 'none') startAurora();
                else stopAurora();
            });
        ...
        (() => {
            ...
            for (let i = 0; i < ORB_COUNT; i++) {
                const el = document.createElement('div');
                el.className = 'ai-gravity-orb';
                const size = 40 + Math.random() * 60;
                el.style.width = size + 'px';
                el.style.height = size + 'px';
                gravityWrap.appendChild(el);
                orbs.push({
                    el,
                    x: Math.random() * 100,
                    y: Math.random() * 100,
            ...
            const renderGravity = () => {
                ...
                orbs.forEach((orb, i) => {
                    ...
                    const orbX = Math.sin(gTime * orb.orbitSpeed + orb.phase) * orb.orbitRadius;
                    const orbY = Math.cos(gTime * orb.orbitSpeed * 0.7 + orb.phase * 1.3) * orb.orbitRadius * 0.6;

                    // Soft inter-orb attraction
                    let fx = 0, fy = 0;
                    for (let j = 0; j < ORB_COUNT; j++) {
                        ...
                        // Attract gently when far, repel softly when close
                        const force = dist > 25 ? 0.0004 * orbs[j].mass / (dist * dist) : -0.002 / (dist + 1);
                        fx += dx * force;
                        fy += dy * force;
                    }

                    orb.vx = (orb.vx + fx) * 0.992;
                    orb.vy = (orb.vy + fy) * 0.992;

                    orb.x += orb.vx + orbX * 0.006;
                    orb.y += orb.vy + orbY * 0.006;

                    // Soft boundary wrap
                    if (orb.x < -8) orb.vx += 0.02;
                    if (orb.x > 108) orb.vx -= 0.02;
                    if (orb.y < -8) orb.vy += 0.02;
                    if (orb.y > 108) orb.vy -= 0.02;
                    orb.x = Math.max(-15, Math.min(115, orb.x));
                    orb.y = Math.max(-15, Math.min(115, orb.y));

                    // Organic breathing scale
                    const breathe = 0.9 + Math.sin(gTime * 0.6 + orb.phase) * 0.1;
                    // Morphing border radius for organic feel
                    const r1 = 42 + Math.sin(gTime * 0.8 + orb.phase) * 10;
                    const r2 = 48 + Math.cos(gTime * 0.6 + orb.phase * 1.2) * 8;
                    const r3 = 45 + Math.sin(gTime * 0.9 + orb.phase * 0.8) * 9;
                    const r4 = 50 + Math.cos(gTime * 0.7 + orb.phase * 1.5) * 7;

                    const color = colors[i % colors.length];
                    const baseOpacity = isDark ? 0.18 : 0.22;
                    const opacity = baseOpacity + Math.sin(gTime * 0.5 + orb.phase) * 0.06;
                    const blur = 28 + Math.sin(gTime * 0.4 + orb.phase) * 6;

                    orb.el.style.cssText = `
                        left: ${orb.x}%;
                        top: ${orb.y}%;
                        width: ${orb.size}px;
                        height: ${orb.size}px;
                        ...
                        transform: translate(-50%, -50%) scale(${breathe.toFixed(3)});
                        border-radius: ${r1.toFixed(0)}% ${r2.toFixed(0)}% ${r3.toFixed(0)}% ${r4.toFixed(0)}%;
                ...
                gRaf = requestAnimationFrame(renderGravity);
            };

            const startGravity = () => { if (!gRaf) renderGravity(); };
            const stopGravity = () => { if (gRaf) { cancelAnimationFrame(gRaf); gRaf = null; } };
        ...
        (() => {
            ...
            const vCanvas = document.createElement('canvas');
            vortexWrap.appendChild(vCanvas);
            panel.insertBefore(vortexWrap, panel.firstChild);
            ...
            const vortexResize = () => {
                const r = panel.getBoundingClientRect();
                vCanvas.width = Math.round(r.width * 0.25);
                vCanvas.height = Math.round(r.height * 0.25);
            };
            ...
            // Silk vortex: layered spiral flow fields with smooth blending
            const renderVortex = () => {
                const ctx = vCanvas.getContext('2d');
                if (!ctx || !vCanvas.width) { vortexRaf = requestAnimationFrame(renderVortex); return; }
                const w = vCanvas.width, h = vCanvas.height;
                ctx.clearRect(0, 0, w, h);
                vTime += 0.003;
                ...
                // Two vortex centers that drift slowly
                const cx1 = 0.35 + Math.sin(vTime * 0.7) * 0.15;
                const cy1 = 0.45 + Math.cos(vTime * 0.5) * 0.2;
                const cx2 = 0.65 + Math.cos(vTime * 0.6 + 2.0) * 0.15;
                const cy2 = 0.55 + Math.sin(vTime * 0.4 + 1.5) * 0.18;
                ...
                for (let y = 0; y < h; y += step) {
                    ...
                    for (let x = 0; x < w; x += step) {
                        ...
                        // Distance and angle to each vortex center
                        const dx1 = nx - cx1, dy1 = ny - cy1;
                        ...
                        const spiral1 = Math.sin(angle1 * 3 + dist1 * 12 - vTime * 1.8) * 0.5 + 0.5;
                        const spiral2 = Math.sin(angle2 * 2 - dist2 * 10 + vTime * 1.4 + 1.0) * 0.5 + 0.5;

                        // Radial fade for softness
                        const fade1 = Math.max(0, 1 - dist1 * 1.8);
                        const fade2 = Math.max(0, 1 - dist2 * 1.8);

                        // Combine spirals with gentle interference
                        const v = spiral1 * fade1 * 0.55 + spiral2 * fade2 * 0.45;

                        // Color blend
                        const ci = v * (rgbs.length - 1);
                        const ci0 = Math.floor(ci);
                        const ci1 = Math.min(ci0 + 1, rgbs.length - 1);
                        const t = ci - ci0;
                        const r = Math.round(rgbs[ci0][0] * (1 - t) + rgbs[ci1][0] * t);
                ...
                ctx.putImageData(imgData, 0, 0);
                vortexRaf = requestAnimationFrame(renderVortex);
            };

            const startVortex = () => { vortexResize(); if (!vortexRaf) renderVortex(); };
            const stopVortex = () => { if (vortexRaf) { cancelAnimationFrame(vortexRaf); vortexRaf = null; } };

            const vortexObs = new MutationObserver(() => {
        ...
        // 事件绑定（仅在没有拖拽时触发点击）— 无延迟丝绒弹簧过渡
        floatBtn.onclick = (e) => {
            if (_dragState.moved) { _dragState.moved = false; return; }
            // 悬浮按钮丝绒缩放退场（不阻塞面板弹出）
            floatBtn.style.transition = 'transform 0.35s cubic-bezier(0.14, 1.40, 0.30, 1.00), opacity 0.28s cubic-bezier(0.04, 0.86, 0.08, 1.00), filter 0.28s ease';
            floatBtn.style.transform = 'scale(0.5) rotate(15deg)';
            floatBtn.style.opacity = '0';
            floatBtn.style.filter = 'blur(6px)';
        ...
        document.getElementById('ai-close-btn').onclick = () => {
            ...
            setTimeout(() => {
                ...
                floatBtn.style.animation = 'ai-velvet-spring-in 0.55s cubic-bezier(0.14, 1.40, 0.30, 1.00)';
            }, 400);
        };
        ...
        // v0.0.7: 工具按钮弹性交错入场索引
        panel.querySelectorAll('.ai-btn-tool').forEach((btn, i) => {
            btn.style.setProperty('--btn-i', i);
        });

        // 首次使用提示：仅在控制台提示，不再自动弹出面板（避免触发画布动效导致页面卡顿/闪烁）
        if (!settings.apiKey) {
            console.log('[AI整理] 首次使用，请点击左下角悬浮按钮打开面板并填入 API Key');
        }
