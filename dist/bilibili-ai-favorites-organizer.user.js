  ...
  function FloatButton($$anchor, $$props) {
    ...
    user_effect(() => {
      if (visible() && wasHidden && get(btnEl)) {
        const saved = gmGetValue("bfao_pos_v5", null);
        if (saved) {
    ...
    onMount(() => {
      ...
      ctx = gsap.context(
        () => {
          _Draggable.create(get(btnEl), {
bounds: document.body,
            edgeResistance: 0.75,
            inertia: false,
            minimumMovement: 8,
            onDragEnd() {
              const tx = gsap.getProperty(get(btnEl), "x");
              const ty = gsap.getProperty(get(btnEl), "y");
...
响应片段: ${snippet2}`));
  ...
  function SettingsPanel($$anchor, $$props) {
    ...
    SettingsGroup(node_1, {
      ...
      children: ($$anchor2, $$slotProps) => {
        ...
        each(node_2, 1, () => AI_CHUNK_PRESETS, index, ($$anchor3, preset) => {
          var option = root_3$a();
          var text2 = child(option);
          var option_value = {};
          template_effect(() => {
        ...
        each(select_1, 5, () => SPEED_PRESETS, index, ($$anchor3, preset) => {
          var option_2 = root_4$a();
          var text_1 = child(option_2);
          var option_2_value = {};
          template_effect(() => {
            set_text(text_1, get(preset).label);
            if (option_2_value !== (option_2_value = get(preset).value)) {
              option_2.value = (option_2.__value = get(preset).value) ?? "";
            }
          });
        ...
        var node_3 = sibling(label, 2);
        {
          var consequent = ($$anchor3) => {
            var input_3 = root_5$8();
            action(input_3, ($$node) => focusGlow == null ? void 0 : focusGlow($$node));
            template_effect(() => set_value(input_3, $settings().limitCount));
            delegated("input", input_3, (e) => settings.update({ limitCount: Number(e.target.value) || 200 }));
        ...
        option_4.value = option_4.__value = 80;
        var option_5 = sibling(option_4);
        option_5.value = option_5.__value = 100;
        var option_6 = sibling(option_5);
        option_6.value = option_6.__value = 150;
        var option_7 = sibling(option_6);
        option_7.value = option_7.__value = 200;
        var option_8 = sibling(option_7);
        option_8.value = option_8.__value = 300;
        var select_2_value;
        init_select(select_2);
        var div_8 = sibling(div_7, 2);
        var select_3 = sibling(child(div_8), 2);
        var option_9 = child(select_3);
        option_9.value = option_9.__value = 0.5;
        var option_10 = sibling(option_9);
        option_10.value = option_10.__value = 1;
        var option_11 = sibling(option_10);
        option_11.value = option_11.__value = 1.5;
        var option_12 = sibling(option_11);
        option_12.value = option_12.__value = 2;
        var option_13 = sibling(option_12);
        option_13.value = option_13.__value = 3;
        var option_14 = sibling(option_13);
        option_14.value = option_14.__value = 5;
        var select_3_value;
        init_select(select_3);
        template_effect(() => {
          if (select_value !== (select_value = $settings().aiChunkSize)) {
            select.value = (select.__value = $settings().aiChunkSize) ?? "", select_option(select, $settings().aiChunkSize);
          }
          if (select_1_value !== (select_1_value = $settings().fetchDelay)) {
            select_1.value = (select_1.__value = $settings().fetchDelay) ?? "", select_option(select_1, $settings().fetchDelay);
          }
          set_value(input, $settings().writeDelay);
          set_value(input_1, $settings().moveChunkSize);
          set_checked(input_2, $settings().limitEnabled);
          if (select_2_value !== (select_2_value = $settings().batchRestInterval)) {
            select_2.value = (select_2.__value = $settings().batchRestInterval) ?? "", select_option(select_2, $settings().batchRestInterval);
          }
          if (select_3_value !== (select_3_value = $settings().batchRestMinutes)) {
            select_3.value = (select_3.__value = $settings().batchRestMinutes) ?? "", select_option(select_3, $settings().batchRestMinutes);
          }
        });
        delegated("change", select, (e) => settings.update({ aiChunkSize: Number(e.target.value) }));
        delegated("change", select_1, (e) => settings.update({ fetchDelay: Number(e.target.value) }));
        delegated("input", input, (e) => settings.update({ writeDelay: Number(e.target.value) || 2500 }));
        delegated("input", input_1, (e) => settings.update({ moveChunkSize: Number(e.target.value) || 20 }));
        delegated("change", input_2, (e) => settings.update({ limitEnabled: e.target.checked }));
        delegated("change", select_2, (e) => settings.update({ batchRestInterval: Number(e.target.value) }));
        delegated("change", select_3, (e) => settings.update({ batchRestMinutes: Number(e.target.value) }));
        append($$anchor2, div_1);
      },
      $$slots: { default: true }
    });
    var node_4 = sibling(node_1, 2);
    SettingsGroup(node_4, {
      title: "行为设置",
      get icon() {
        return Toggle_right;
      },
      ...
      children: ($$anchor2, $$slotProps) => {
        ...
        var node_9 = sibling(div_13, 2);
        {
          var consequent_1 = ($$anchor3) => {
            ...
            option_15.value = option_15.__value = 5;
            var option_16 = sibling(option_15);
            option_16.value = option_16.__value = 15;
            var option_17 = sibling(option_16);
            option_17.value = option_17.__value = 30;
            var option_18 = sibling(option_17);
            option_18.value = option_18.__value = 60;
            var select_4_value;
            init_select(select_4);
            template_effect(() => {
              if (select_4_value !== (select_4_value = $settings().cacheScanInterval)) {
                select_4.value = (select_4.__value = $settings().cacheScanInterval) ?? "", select_option(select_4, $settings().cacheScanInterval);
              }
            });
            delegated("change", select_4, (e) => settings.update({ cacheScanInterval: Number(e.target.value) }));
    ...
    SettingsGroup(node_10, {
      ...
      children: ($$anchor2, $$slotProps) => {
        ...
        var node_11 = sibling(child(div_15), 2);
        LiquidToggle(node_11, {
          label: "启用极致动画效果",
          get checked() {
  ...
  var root$b = from_html(`<div class="prompt-editor svelte-avw5xh"><div class="prompt-header svelte-avw5xh"><select class="bfao-select svelte-avw5xh"><!><optgroup label="内置预设" class="svelte-avw5xh"></optgroup></select> <button class="prompt-action-btn svelte-avw5xh" title="保存为自定义预设"><!></button> <button class="prompt-action-btn svelte-avw5xh" title="管理预设"><!></button></div> <textarea class="prompt-textarea svelte-avw5xh" placeholder="输入自定义整理规则（留空则 AI 自动判断最佳分类方式）  示例：按游戏类型分类，如 MOBA、FPS、RPG..."></textarea> <!></div>`);
  function PromptEditor($$anchor, $$props) {
      ...
      var consequent_3 = ($$anchor2) => {
        ...
        var node_4 = sibling(child(div_2), 2);
        {
          var consequent_1 = ($$anchor3) => {
            ...
            each(node_5, 17, () => get(customPresets), (preset) => preset.id, ($$anchor4, preset) => {
              var div_3 = root_6$3();
        ...
        each(node_8, 17, () => BUILTIN_PRESETS, index, ($$anchor3, preset) => {
          var div_4 = root_7$2();
          ...
          template_effect(
            ($0) => {
              classes = set_class(button_6, 1, "preset-select svelte-avw5xh", null, classes, $0);
    ...
    delegated("click", button_1, () => set(showManager, !get(showManager)));
...
life: 0,
  ...
  var root_5$3 = from_html(`<div class="health-score svelte-l1lb8j"><svg class="health-ring svelte-l1lb8j" viewBox="0 0 120 120" width="120" height="120"><circle cx="60" cy="60" r="54" fill="none" stroke="var(--ai-border-lighter)" stroke-width="7"></circle><circle cx="60" cy="60" r="54" fill="none" stroke-width="7" stroke-linecap="round" transform="rotate(-90 60 60)"></circle></svg> <div class="score-overlay svelte-l1lb8j"><div class="score-number svelte-l1lb8j"> </div> <div class="score-label svelte-l1lb8j">健康评分</div></div></div> <div class="health-detail svelte-l1lb8j"><!></div>`, 1);
  ...
  var root_4$3 = from_html(`<div class="bfao-modal-body"><!> <div class="stats-grid svelte-l1lb8j"><div class="stat-card svelte-l1lb8j"><div class="stat-value svelte-l1lb8j"> </div> <div class="stat-label svelte-l1lb8j">收藏夹</div></div> <div class="stat-card svelte-l1lb8j"><div class="stat-value svelte-l1lb8j" data-locale="true"> </div> <div class="stat-label svelte-l1lb8j">视频总数</div></div> <div class="stat-card svelte-l1lb8j"><div> </div> <div class="stat-label svelte-l1lb8j">失效视频</div></div> <div class="stat-card svelte-l1lb8j"><div class="stat-value svelte-l1lb8j"> </div> <div class="stat-label svelte-l1lb8j">失效率</div></div></div> <!></div>`);
  ...
  var root_5$2 = from_html(`<label><input type="checkbox"/> <div class="folder-info svelte-184y11r"><div class="folder-title svelte-184y11r"> </div> <div class="folder-count svelte-184y11r"> </div></div></label>`);
      ...
      Modal($$anchor, {
        ...
        children: ($$anchor2, $$slotProps) => {
          ...
          each(div_2, 21, folders, (folder) => folder.id, ($$anchor3, folder) => {
            ...
            template_effect(
              ($02, $1) => {
                classes = set_class(label, 1, "bfao-selectable-item", null, classes, $02);
                set_checked(input, $1);
  ...
  function VideoItem($$anchor, $$props) {
    ...
    template_effect(() => {
      var _a2, _b2;
      classes = set_class(div, 1, "video-item svelte-9tq2cj", null, classes, { "virtual-item": virtual() });
      styles = set_style(div, "", styles, { top: virtual() ? `${top()}px` : void 0 });
  ...
  var root_3$2 = from_html(`<div class="video-list virtual-scroll svelte-t28wkk"><div class="virtual-spacer svelte-t28wkk"></div></div>`);
  var root_5 = from_html(`<div class="video-list svelte-t28wkk"></div>`);
      ...
      var consequent_2 = ($$anchor2) => {
        ...
        var node_4 = first_child(fragment);
        {
          var consequent_1 = ($$anchor3) => {
            ...
            var div_3 = child(div_2);
            let styles_1;
            each(div_3, 23, () => $$props.vids.slice($$props.visibleRange.start, $$props.visibleRange.end), (vid) => vid.id, ($$anchor4, vid, i) => {
              {
            ...
            template_effect(() => {
              styles = set_style(div_2, "", styles, { height: `${visibleRows() * rowHeight()}px` });
              styles_1 = set_style(div_3, "", styles_1, { height: `${$$props.vids.length * rowHeight()}px` });
            });
          ...
          var alternate = ($$anchor3) => {
            var div_4 = root_5();
            each(div_4, 21, () => $$props.vids, (vid) => vid.id, ($$anchor4, vid) => {
              {
                let $0 = user_derived(() => $$props.videoMap.get(get(vid).id));
                VideoItem($$anchor4, {
                  get vid() {
  ...
  function PreviewToolbar($$anchor, $$props) {
    ...
    template_effect(() => {
      ...
      classes_4 = set_class(button_4, 1, "filter-btn svelte-1wb2xl2", null, classes_4, { active: $$props.mergeMode });
...
4. 绝不遗漏任何视频${existingPart}${customPart}`.trim();
  ...
  async function fetchSourceVideos(sourceMediaIds, settings2, isCancelled) {
    ...
    const estimatedTotal = maxVideos ?? sourceMediaIds.length * 100;
    let fetchedSoFar = 0;
...
${JSON.stringify(videoData)}`
  ...
  function Panel($$anchor, $$props) {
    ...
    function savePosition() {
      ...
      const tx = gsap.getProperty(get(panelEl), "x");
      const ty = gsap.getProperty(get(panelEl), "y");
      gmSetValue(POS_KEY, { tx, ty });
    }
    onMount(() => {
      const saved = gmGetValue(POS_KEY, null);
      if (saved) {
        ...
        () => {
          if (flipState() && shouldAnimateFunctional()) {
            ...
            gsap.from(get(panelEl), {
              ...
              ease: EASINGS.velvetSpring,
              onComplete() {
                get(panelEl).style.removeProperty("filter");
              }
            });
